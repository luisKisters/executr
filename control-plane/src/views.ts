...
import { DEFAULT_CONTROL_PLANE_PROVIDER, type ClassificationSignal, type ProviderName } from './contracts';
...
export function renderNewPlanPage(
  ...
  const providerOptions = providers.map(p =>
    `<option value="${p}"${(values.provider ?? DEFAULT_CONTROL_PLANE_PROVIDER) === p ? ' selected' : ''}>${p}</option>`
  ).join('');
