...
describe('createPlan — provider and claims', () => {
  ...
  it('submitRawPlan defaults to a codex claim', () => {
    const markdown = '# Plan: Raw Default Provider\n\n## Validation Commands\n\n```\npnpm test\n```\n\n### Task 1: Raw\n\n- [ ] Submit it\n';
    const outcome = submitRawPlan({ workspaceRoot, claimsDir, repo: 'testrepo', markdown });
    expect(outcome.ok).toBe(true);
    if (!outcome.ok) return;
